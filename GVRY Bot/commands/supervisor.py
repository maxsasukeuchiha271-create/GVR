...
class Supervisor(commands.Cog):
    ...
    @app_commands.command(name="supervisor", description="Announce yourself as the supervisor for the session")
    async def supervisor(self, interaction: discord.Interaction):
        if 1474665732133486592 not in [role.id for role in interaction.user.roles]:
        ...
        await interaction.response.send_message("You have been announced as the supervisor.", ephemeral=True)
        await interaction.channel.send(embed=embed)

async def setup(bot):
    await bot.add_cog(Supervisor(bot))
